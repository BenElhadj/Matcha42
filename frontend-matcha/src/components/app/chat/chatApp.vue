        ...
        <div class="inbox_people hidden-sm-and-down">
          ...
          <div class="inbox_chat">
            <div v-for="(btn, index) in btns" :key="index" class="chat_list active_chat">
              <div class="chat_people" @click="chatId(btn.id)">
                <div class="chat_img">
                  <v-avatar>
                    <img :src="btn.img" />
                  </v-avatar>
...
<!--                   <input v-model="msg" type="text" class="write_msg" placeholder="Type a message" /> -->
                  <v-btn icon class="msg_send_btn" @click="sendMsg">
...
export default {
  ...
  created() {
    console.log("Je suis dans le composant @/components/app/chat/chatApp.vue");
  },
  ...
  setup() {
    ...
    const chatId = (id) => {
      yourid.value = id;
      const tmp = this.token;
      const obj = {};
      obj.id = id;
      obj.tmp = tmp;

      api.post("/chat/getimage", obj).then(data => {
        if (data.data == "Logout") {
      ...
      api.post("/chat/datamsg", obj).then(data => {
        if (data.data == "Logout") {
          this.$cookies.removeAll();
          this.$router.push("/");
        }
    ...
    const sendMsg = async () => {
      ...
      obj.tmp = tmp;
      obj.msg = this.msg;
      api.post("/chat/sendmsg", obj).then(data => {
        if (data.data == "Logout") {
          this.$cookies.removeAll();
          this.$router.push("/");
        }
    ...
    const sortBy = (prop) => {
      msgs.value.sort((a, b) => a[prop] < b[prop] ? -1 : 1)
    };
  ...
  onMounted(() => {
    ...
    const tmp = this.$cookies.get("token");
    const obj = { tmp: tmp };
    api.post("/match/friends", obj).then(data => {
        if (data.data == "Logout") {
          this.$cookies.removeAll();
          this.$router.push("/");
        }
