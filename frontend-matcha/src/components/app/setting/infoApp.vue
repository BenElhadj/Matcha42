    ...
    <v-col>
        ...
        <v-card color="accent">
            <v-form v-model="valid">
                ...
                <v-row  style="margin: 2vh 0 0 0">
                    <v-container fluid>
                        <v-combobox eager v-model="Tags" :items="items" v-model:search-input="search" hide-selected label="Add some tags" multiple persistent-hint small-chips :rules="TagsRules"></v-combobox>
                    </v-container>
                </v-row>
                <v-row  style="margin: 2vh 0 0 0">
                ...
                <v-row  style="margin: 2vh 0 0 0; padding: 0 0 2vh 0;">
                    <v-btn color="primary" @click="await_update" width="95%" style="margin: 0 1vh 0 1vh" :disabled="!valid">Save</v-btn>
                </v-row>
            </v-form>            
        </v-card>
...
export default {
    name: "InfoApp",
    created() {
    console.log("Je suis dans le composant @/components/app/setting/infoApp.vue");
    },
    data: () => ({
        snackbar: false,
        timeout: 8000,
    ...
    methods: {
        await_update: async function() {
            const respons = await this.update();
            this.snackbar = true;
